...
def expMultisite(currents):
    ...
    Stim['dur'] = 0.05 # [ms]
    Stim['stop'] = 50 # Old: 60
    Stim['amp'] = [currents[0],currents[1],currents[2],currents[3]]

    Stim['initDur'] = 50
    Stim['initDt'] = 1
    Stim['vInit'] = -70 # units: [mV]

    Stim['electrodes'] = [[-1060,0,42],[-1030,15,42],[-1030,-15,42],[-1000,0,42]]
    ...
    dir = '/Volumes/Lab/Users/vilkhu/data-backup/sim-data/quad_elec/v1/'
    ...
    filename = dir+str(currents[0])+'_'+str(currents[1])+'_'+str(currents[2])+'_'+str(currents[3])+'.pkl'
...
def main(): 
    ...
    pool = mp.Pool(25)
    print('Testing quad elecs')

    # ## --- create a list of currents to test; units: [uA] (2-electrode, full)
    currents = []
    current_range = np.linspace(-4,4,18)
    current_range = np.round(current_range, 2)
    for i1 in current_range:
        for i2 in current_range:
            for i3 in current_range:
                for i4 in current_range:
                    currents.append([round(i1,2),round(i2,2),round(i3,2),round(i4,2)])
