...
def expMultisite(angle):
    unit_curr_dir = np.array([-1,1])
    ...
    elec1_rounded = [round(elec1_rotated[0], 2), round(elec1_rotated[1], 2),42]
    elec2_rounded = [round(elec2_rotated[0], 2), round(elec2_rotated[1], 2),42]


    Stim['electrodes'] = [elec1_rounded,elec2_rounded] #[[-1030,0,47],[-1000,0,47]]
    ...
    upBound = 15 # TODO: Changed from 5
    stop = False
    scale_factor = np.nan
    ...
    if scale_factor == np.nan:
        ...
        dir = '/Volumes/Lab/Users/vilkhu/data-backup/sim-data/bielec_rotations/soma_60um/'

        # Second, save spiking information (spike, init compartment)
        filename = dir+str(angle)+'.pkl'
        os.makedirs(os.path.dirname(filename), exist_ok=True)
        data = np.asarray([currents], dtype=object)
        try: 
...
def main(): 
    ...
    pool = mp.Pool(20)
    print('Testing bielec thresholds for various rotations, fixed ratio -1, 1 currents')

    angles = []
    # ## --- create a list rotations
    for angle in range(0, 91, 5):
        angles.append(angle)

    ### --- run parallel simulations for each electrode location
    result = list(tqdm.tqdm(pool.imap(expMultisite, angles), total=len(angles)))
