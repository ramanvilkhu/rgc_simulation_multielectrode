...
def expMultisite(currents):
    ...
    Stim['dur'] = 0.05 # [ms]
    Stim['stop'] = 50 # Old: 60
    Stim['amp'] = [currents[0],currents[1],currents[2]]

    Stim['initDur'] = 50
    Stim['initDt'] = 1
    Stim['vInit'] = -70 # units: [mV]

    Stim['electrodes'] = [[20,-20,42],[5,10,42],[35,10,42]]
    ...
    dir = '/Volumes/Lab/Users/vilkhu/data-backup/sim-data/triplet_somatic_rotations/90deg/'
...
def main(): 
    ...
    pool = mp.Pool(12)
    print('Testing somatic triplet at various rotations: 90deg') # 0, 10, 20, 30, 40

    # ## --- create a list of currents to test; units: [uA] (2-electrode, full)
    currents = []
    current_range = np.linspace(-4,4,20)
    ...
    for i1 in current_range:
        for i2 in current_range:
            ...
            #     currents.append([round(i1,2),round(i2,2)])
                currents.append([round(i1,2),round(i2,2),round(i3,2)])
