...
def expMultisite(electrodes):
    ...
    for ii,unit_curr_dir in enumerate(unit_vectors):
        print('Working on electrode: ', electrodes, ' and direction: ', unit_curr_dir)

        # Implement a bisection search to find the optimal scaling factor
        epsilon = 0.05
        lowBound = 0.5
        upBound = 7 # TODO: Changed from 5
        stop = False
        scale_factor = np.nan
        ...
        if scale_factor == np.nan:
            ...
            # determine (x,y) center between elecs
            elec1 = electrodes[0]
            elec2 = electrodes[1]
            elec_center = [(elec1[0]+elec2[0])/2, (elec1[1]+elec2[1])/2]
            dir_name = 'rot0_'+str(elec_center[0])+'_'+str(elec_center[1])
            dir = '/Volumes/Lab/Users/vilkhu/data-backup/nonlinearity_final_v2/'+dir_name+'/'
...
def main(): 
    ...
    # pool = mp.Pool(10)
    print('Testing nonlinearity for various electrode locs, saving to final...')
    ...
    result = list(tqdm.tqdm(pool.imap(expMultisite, electrodes), total=len(electrodes)))
